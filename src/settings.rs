...
pub struct SettingsProxy {
    ...
    /// automatically start that daemon, along with its `depends` graph in the
    /// same order `pitchfork start` uses. The browser receives a "Starting…"
    /// page that refreshes every 2 seconds until the daemon is ready, at which
    /// point the request is proxied normally.
    ///
    ...
    /// starting its dependencies, waiting for the daemon's readiness signal, and
    ...
    /// The startup itself continues in the background.
    ///
    /// **Examples:**
