...
pub trait SettingsPartialGroup {
...
impl SettingsApiPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsApiPartial {
    fn is_empty(&self) -> bool {
        self.auto_start.is_none()
            && self.bind_address.is_none()
            && self.bind_port.is_none()
            && self.port_attempts.is_none()
            && self.token.is_none()
    }
}

impl SettingsGeneralPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsGeneralPartial {
    fn is_empty(&self) -> bool {
        self.autostop_delay.is_none()
            && self.interval.is_none()
            && self.log_file_level.is_none()
            && self.log_level.is_none()
            && self.mise.is_none()
            && self.mise_bin.is_none()
            && self.startup_log_timestamps.is_none()
    }
}

impl SettingsIpcPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsIpcPartial {
    fn is_empty(&self) -> bool {
        self.connect_attempts.is_none()
            && self.connect_max_delay.is_none()
            && self.connect_min_delay.is_none()
...
impl SettingsLogsPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsLogsPartial {
    fn is_empty(&self) -> bool {
        self.line_retention.is_none() && self.time_retention.is_none()
    }
}

impl SettingsProxyPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsProxyPartial {
    fn is_empty(&self) -> bool {
        self.auto_start.is_none()
            && self.auto_start_timeout.is_none()
            && self.auto_trust.is_none()
            && self.enable.is_none()
            && self.host.is_none()
            && self.https.is_none()
            && self.lan.is_none()
...
impl SettingsSupervisorPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsSupervisorPartial {
    fn is_empty(&self) -> bool {
        self.cleanup_orphans.is_none()
            && self.container.is_none()
            && self.cpu_violation_threshold.is_none()
            && self.cron_check_interval.is_none()
            && self.file_watch_debounce.is_none()
            && self.http_client_timeout.is_none()
            && self.log_flush_interval.is_none()
            && self.port_bump_attempts.is_none()
            && self.ready_check_interval.is_none()
            && self.restart_delay.is_none()
            && self.stop_timeout.is_none()
            && self.user.is_none()
            && self.watch_interval.is_none()
            && self.watch_poll_interval.is_none()
    }
}

impl SettingsTuiPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsTuiPartial {
    fn is_empty(&self) -> bool {
        self.message_duration.is_none()
            && self.refresh_rate.is_none()
            && self.stat_history.is_none()
            && self.tick_rate.is_none()
    }
}

impl SettingsWebPartial {
    pub fn has_any_set(&self) -> bool {
        !self.is_empty()
    }
}

impl SettingsPartialGroup for SettingsWebPartial {
    fn is_empty(&self) -> bool {
        self.auto_start.is_none()
            && self.base_path.is_none()
            && self.bind_address.is_none()
            && self.bind_port.is_none()
            && self.log_lines.is_none()
            && self.port_attempts.is_none()
            && self.sse_poll_interval.is_none()
    }
}
