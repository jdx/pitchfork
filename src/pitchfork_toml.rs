...
impl PitchforkToml {
    ...
    pub fn resolve_daemon_id(&self, user_id: &str) -> Result<Vec<DaemonId>> {
        ...
        if matches.is_empty() {
            ...
            // Return with "global" namespace as default, but validate first
            Ok(vec![DaemonId::try_new("global", user_id)?])
        } else {
            Ok(matches)
        }
    }
    ...
    pub fn resolve_daemon_id_prefer_local(
        ...
        // Warn if there are multiple matches (ambiguity)
        if matches.len() > 1 {
            let namespaces: Vec<&str> = matches.iter().map(|id| id.namespace()).collect();
            warn!(
                "Daemon '{}' exists in multiple namespaces: {}. Using '{}'. \
                 To avoid ambiguity, use the qualified name (e.g., '{}/{}')",
                user_id,
                namespaces.join(", "),
                matches[0],
                namespaces[0],
        ...
        matches.into_iter().next().ok_or_else(|| {
            // This shouldn't happen since resolve_daemon_id creates a default,
            // but handle it gracefully by returning the validation error
            DaemonId::try_new("global", user_id).unwrap_err()
        })
    }
...
impl PitchforkToml {
    ...
    pub fn read<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        for (short_name, raw_daemon) in raw_config.daemons {
            // Validate daemon name from config file - use try_new to get proper error
            let id = match DaemonId::try_new(&namespace, &short_name) {
                Ok(id) => id,
                Err(e) => {
                    // Return error with helpful context about the invalid daemon name
            ...
            for dep in raw_daemon.depends {
                let dep_id = if dep.contains('/') {
                    ...
                    // Same namespace dependency - validate with try_new
                    match DaemonId::try_new(&namespace, &dep) {
                        Ok(id) => id,
                        Err(e) => {
                            warn!(
                                "Invalid dependency '{}' in daemon '{}': {}. Skipping.",
                                dep, short_name, e
                            );
                            continue;
                        }
