            ...
            && let Some(action) = event::handle_event(app)?
        {
            match action {
                ...
                event::Action::Start(id) => {
                    ...
                    let result = client
                        .start_daemons(&[id.clone()], false, StartOptions::default())
                        .await;
                ...
                event::Action::BatchStart(ids) => {
                    ...
                    let result = client
                        .start_daemons(&ids, false, StartOptions::default())
                        .await;
                    ...
                    if let Some(pending) = app.take_pending_action() {
                        match pending {
                            ...
                            app::PendingAction::Restart(id) => {
                                ...
                                let result = client.start_daemons(&[id.clone()], false, opts).await;
                            ...
                            app::PendingAction::BatchStop(ids) => {
                                ...
                                terminal.draw(|f| ui::draw(f, app)).into_diagnostic()?;
                                let result = client.stop_daemons(&ids, false).await;
                                app.stop_loading();
                                app.clear_selection();
                                match result {
                                    Ok(r) if r.any_failed => {
                            ...
                            app::PendingAction::BatchRestart(ids) => {
                                ...
                                let result = client.start_daemons(&ids, false, opts).await;
