...
impl IpcClient {
    ...
    ///
    /// If `ids` is empty and `all` is true, starts all daemons from config.
    /// Handles:
    ...
    pub async fn start_daemons(
        ...
        let requested_ids: Vec<String> = if all {
        ...
        let explicitly_requested: HashSet<String> = if all {
            pt.daemons.keys().cloned().collect()
        } else {
    ...
    fn spawn_start_task(
        ...
        let port = opts.port;
        let cmd_override = opts.cmd.clone();
    ...
    /// - Parallel execution within dependency levels
    ///
    /// If `ids` is empty and `all` is true, stops all running daemons.
    pub async fn stop_daemons(self: &Arc<Self>, ids: &[String], all: bool) -> Result<StopResult> {
        let pt = PitchforkToml::all_merged();
        ...
        // Determine which daemons to stop
        let requested_ids: Vec<String> = if all {
            running_daemons.iter().cloned().collect()
        } else {
