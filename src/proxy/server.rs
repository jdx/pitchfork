...
async fn serve_https_with_http_fallback(
    ...
    let resolver: Arc<dyn rustls::server::ResolvesServerCert> = if s.proxy.tls_cert.is_empty() {
        ...
        Arc::new(StaticCertResolver::new(&cert_path, &key_path)?)
    };
...
impl StaticCertResolver {
    fn new(cert_path: &std::path::Path, key_path: &std::path::Path) -> crate::Result<Self> {
...
struct SniCertResolver {
    ...
    /// TLD this CA is willing to sign for. Anything else is refused.
    tld: String,
