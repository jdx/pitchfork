...
Without namespacing, these would conflict. Pitchfork solves this by automatically qualifying daemon IDs with a namespace derived from the project directory name.
...
The namespace is automatically derived from the directory containing the `pitchfork.toml` file.
...
Pitchfork automatically resolves the short ID to the correct namespaced daemon.
