...
## Flags
- **`--pid <PID>`** — Host process PID that owns the session. Required.
- **`--directory <DIRECTORY>`** — Project directory to associate with the session. Defaults to the current working directory and is canonicalized before tracking.
- **`-h --help`** — Print help
