...
Limits the auto-start operation, including dependency startup, readiness checks, and detection of the daemon's bound port. Defaults to 30 seconds.

On timeout, the request receives an error page and the remaining startup sequence is cancelled. Processes already started are not stopped. Increase this setting for a longer startup sequence, for example `"60s"`.
